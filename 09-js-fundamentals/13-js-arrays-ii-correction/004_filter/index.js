...
// 2.  **Use the `filter` Method to Create a New Array with
// Numbers Greater Than 15:**
//     *   Use the `filter` method to create a new array called
// `numbersGreaterThan15` containing all numbers from `numbers` that are greater than `15`.
//     *   Print the `numbersGreaterThan15` array to the console.
const numbersOver15 = numbers.filter((number) => number > 15);
...
//     *   Use the `filter` method to create a new array called
// `studentsAbove80` containing all students from `students` who scored above `80`.
