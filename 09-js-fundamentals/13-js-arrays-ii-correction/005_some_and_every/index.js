...
const isAnyOver20 = numbers.some((number) => number > 20);

console.log('isAnyOver20:', isAnyOver20);
// 3.  **Use the `every` Method to Check for Numbers Less Than 50:**

//     *   Use the `every` method to check if all numbers in `numbers`
// are less than `50`.
const isEveryUnder50 = numbers.every((number) => number < 50);
//     *   Print the result to the console (should be `true`).
console.log('isEveryUnder50:', isEveryUnder50);
...
//     *   Use the `some` method to check if there are any students in `students`
//  who have `passed` set to `false`.
//     *   Print the result to the console (should be `true`).
// const didAnyFail = students.some((student) => student.passed === false);
...
//     *   Use the `every` method to check if all students in `students`
//  are older than `18`.
//     *   Print the result to the console (should be `true`).
// const areAllOver18 = students.every((student) => student.age > 18);
const areAllOver18 = students.every(({ age }) => age > 18);

console.log('areAllOver18:', areAllOver18);
